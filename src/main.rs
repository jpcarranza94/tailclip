...
fn serve_cmd(args: &[String]) -> Result<(), String> {
    ...
    let port: u16 = match flag_value(args, "--port") {
        Some(p) => p.parse().map_err(|_| format!("--port {p}: not a port number"))?,
...
fn selftest() -> Result<(), String> {
    ...
    server::set_poll_timeout(Duration::from_secs(5));
    ...
    check!(r.status == 200 && r.version == 0 && r.body.is_empty(),
        "1  empty start reports version 0");

    // 2. A first POST bumps the version to 1.
    let r = push(&url, TEXT_MIME, b"hello", short)?;
    check!(r.status == 200 && r.version == 1, "2  first post bumps the version to 1");
    ...
    let r = push(&url, TEXT_MIME, b"hello", short)?;
    check!(r.version == 1, "4  an identical post does not bump the version");
    ...
    let v_png = r.version;
    let r = fetch(&url, None, short)?;
    check!(r.body == png && r.mime.starts_with(PNG_MIME),
        "7  a PNG round trips and keeps its content type");
    check!(matches!(Payload::from_wire(&r.mime, r.body), Payload::Png(_)),
    ...
    // 8. A long poll blocks, then returns 200 when another thread posts.
    {
        ...
        check!(r.status == 200 && r.version == v_png + 1,
            "8  a long poll returns 200 after a new post");
        check!(r.body == b"pushed by the other thread", "8b the long poll carries the new clip");
        check!(waited >= Duration::from_millis(300), "8c the long poll blocked, it waited {}ms",
            waited.as_millis());
    }

    // 9. A long poll that times out returns 204.
    server::set_poll_timeout(Duration::from_millis(500));
    {
        let now = fetch(&url, None, short)?.version;
        let t0 = Instant::now();
        let r = fetch(&url, Some(now), short)?;
        let waited = t0.elapsed();
        check!(r.status == 204 && r.body.is_empty(), "9  a long poll timeout returns 204");
        check!(r.version == now, "9b the 204 carries the current version");
        check!(waited >= Duration::from_millis(400), "9c the timeout waited {}ms",
            waited.as_millis());
    }
    server::set_poll_timeout(Duration::from_secs(5));
    ...
    // 10. A body larger than 32 MB returns 413.
    {
        ...
        check!(r.status == 413, "10 a body larger than 32 MB returns 413, got {}", r.status);
        let r = fetch(&url, None, short)?;
        check!(r.body != too_big, "10b the oversize clip was not stored");
    }

    // 11. The bind address rules.
    check!(is_private("100.110.80.23".parse::<IpAddr>().unwrap()),
        "11 the rules accept the Tailscale address 100.110.80.23");
    check!(pick_bind(Some("100.110.80.23"), false).is_ok(), "11b --bind accepts a tailnet address");
    check!(pick_bind(Some("8.8.8.8"), false).is_err(), "11c --bind rejects a public address");
    check!(pick_bind(Some("8.8.8.8"), true).is_ok(),
        "11d --allow-public-bind overrides the rejection");
    check!(pick_bind(Some("192.168.1.10"), false).is_ok(), "11e --bind accepts an RFC1918 address");
    check!(pick_bind(Some("127.0.0.1"), false).is_ok(), "11f --bind accepts loopback");
    check!(pick_bind(Some("not-an-ip"), false).is_err(), "11g --bind rejects a name");
